  ...
  <div className="border-alpha/5 flex items-center border-b p-2" cmdk-input-wrapper="">
    <div className="bg-alpha/3 border-alpha/3 flex w-full items-center gap-1 rounded-lg border p-1.5 focus-within:border-blue-500/70 focus-within:ring-4 focus-within:ring-blue-500/10">
      ...
      <CommandPrimitive.Input
        ref={ref}
        className={cn(
          "flex w-full bg-transparent text-sm font-medium outline-none disabled:cursor-not-allowed disabled:opacity-50",
          className,
        )}
        {...props}
      />
...
>(({ className, variant = "default", ...props }, ref) => (
  <CommandPrimitive.Empty
    ...
    className={cn(
      variant === "default"
        ? "flex h-12 items-center justify-center px-1.5 text-sm text-gray-950/50"
        : "",
      className,
    )}
    {...props}
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.Separator
    ref={ref}
    className={cn("bg-alpha/5 -my-1 h-px", className)}
    {...props}
  />
));
